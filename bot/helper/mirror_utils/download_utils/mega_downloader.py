...
def add_mega_download(mega_link: str, path: str, listener):
    ...
    if STOP_DUPLICATE and not listener.isLeech:
        ...
        if mname is not None:
            ...
            if smsg:
                msg1 = f"⚠️ {listener.tag} File/Folder sudah ada di Drive."
    ...
    if ZIP_UNZIP_LIMIT is not None and (listener.isZip or listener.extract):
        msg3 = f'Gagal, Zip/Unzip limit {ZIP_UNZIP_LIMIT}GB.\nUkuran File/Folder kamu adalah {get_readable_file_size(api.getSize(node))}.'
        limit = ZIP_UNZIP_LIMIT
    elif MEGA_LIMIT is not None:
        msg3 = f'Gagal, Mega limit {MEGA_LIMIT}GB.\nUkuran File/Folder kamu adalah {get_readable_file_size(api.getSize(node))}.'
        limit = MEGA_LIMIT
