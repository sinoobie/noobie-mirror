...
def add_qb_torrent(link, path, listener, select):
    ...
    try:
        ...
        if len(tor_info) > 0:
            sendMessage(f"ℹ️ {listener.tag} Torrent ini sudah ada di task.", listener.bot, listener.message)
            client.auth_log_out()
            return
        if is_file:
        ...
        if op.lower() == "ok.":
            ...
            if len(tor_info) == 0:
                while True:
                    if time() - add_time >= 30:
                        ermsg = f"⚠️ {listener.tag} The Torrent was not added. Report when you see this error"
            ...
            sendMessage(f"⚠️ {listener.tag} unsupported/invalid link.", listener.bot, listener.message)
            client.torrents_delete(torrent_hashes=ext_hash, delete_files=True)
            client.auth_log_out()
            return
        tor_info = tor_info[0]
        ...
        if BASE_URL is not None and select:
            if not is_file:
                metamsg = f"ℹ️ {listener.tag} Downloading Metadata, Tunggu sebentar!"
            ...
            buttons.sbutton("Selesai Memilih", f"qbs done {gid} {ext_hash}")
            QBBUTTONS = InlineKeyboardMarkup(buttons.build_menu(2))
            msg = f"ℹ️ {listener.tag} Download kamu dijeda. Pilih file lalu klik \"Selesai Memilih\" untuk memulai download."
            sendMarkup(msg, listener.bot, listener.message, QBBUTTONS)
        else:
            sendStatusMessage(listener.message, listener.bot)
    except Exception as e:
        sendMessage(f"⚠️ {listener.tag} {e}", listener.bot, listener.message)
        client.auth_log_out()
