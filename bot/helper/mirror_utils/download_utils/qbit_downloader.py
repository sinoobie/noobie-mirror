...
class QbDownloader:
    ...
    def add_qb_torrent(self, link, path, select):
        ...
        try:
            ...
            if len(tor_info) > 0:
                sendMessage(f"ℹ️ {self.__listener.tag} Torrent tersebut sudah ada di task.", self.__listener.bot, self.__listener.message)
            ...
            if op.lower() == "ok.":
                ...
                if len(tor_info) == 0:
                    while True:
                        if time() - add_time >= 30:
                            ermsg = f"⚠️ {self.__listener.tag} The Torrent was not added. Report when you see this error"
