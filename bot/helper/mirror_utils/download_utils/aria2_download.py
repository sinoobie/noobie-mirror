        ...
        if any([STOP_DUPLICATE, TORRENT_DIRECT_LIMIT, ZIP_UNZIP_LIMIT, STORAGE_THRESHOLD]):
            ...
            if STOP_DUPLICATE and not dl.getListener().isLeech:
                ...
                if sname is not None:
                    ...
                    if smsg:
                        dl.getListener().onDownloadError(f'<code>{sname}</code> sudah ada di Drive.', markup=True, button=button)
...
def __onDownloadError(api, gid):
    ...
    try:
        ...
        pass
    if dl:
        dl.getListener().onDownloadError(f"Oops terjadi error atau sepertinya link kamu bukan direct link.\n\n<code>aria2_onDownload_error: {error}</code>")
